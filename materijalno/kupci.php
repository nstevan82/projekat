...
if (isset($_POST['unesi'])){
	   ...
	   $rezultat= $mysqli->query("INSERT INTO rac_kupci (naziv,sifra,pib,adresa,telefon,aktivan) VALUES ('$naziv','$sifra','$pib','$adresa','$telefon',1)");
		...
		$telefon=$_POST['telefon'];
		$id=$_POST['id'];
		$rezultat= $mysqli->query("UPDATE rac_kupci SET naziv='$naziv',sifra='$sifra',pib='$pib',adresa='$adresa',telefon='$telefon' WHERE id='$id'");
	}
}	
	
	
	if (isset($_REQUEST['del'])) {
		$rezultat= $mysqli->deaktiviraj($_REQUEST['del'],"rac_kupci");
		...
		$pocetak=$mysqli->paginacija1("SELECT COUNT(ID) AS broj FROM rac_kupci WHERE aktivan=1","kupci");
		$rezultat= $mysqli->query("SELECT * FROM rac_kupci WHERE aktivan=1 LIMIT $pocetak,10");
	  	?>
		...
		<div class="box">
			<table class="lista" style="diplay:block;width:100%">
				...
				$id=$row['id'];
				$naziv=$row['naziv'];
				$link="default.php?str=kupci&id=$id";
				
				echo "<tr class='lista'><td>".$i."</td><td><a href='$link'>".$row['naziv']."</a></td><td><a href='$link'>".$row['sifra']."</a></td><td><a href='default.php?str=kupci&del=$id'>x</a></td></tr>";
				$i=$i+1;
				}
				?>
			</table>
...
// a ako je korisnik kliknuo na odredjeni red iscitaj vrednosti iz baze
		...
		if (isset($_GET['id'])){
		$id=$_GET['id'];
		$rezultat= $mysqli->query("SELECT * FROM rac_kupci WHERE id='$id'");
		while ($row = $rezultat->fetch_assoc()) 
				{
				$id1=$row['id'];
				$naziv1=$row['naziv'];
...
<form method="post" action="default.php">
		...
		<input type="hidden" value=kupci name="str">
