...
<div class="box">
<?php
$_GET['dobavljaci']=($_POST['dobavljaci'])+1;
		...
		<input type="hidden" name=str value="poc_ulazne">
...
$godina=$_POST['godina'];
	...
	if (isset($_POST['resetuj']) && $_POST['resetuj']=='on' ) $rezultat= $mysqli->query("DELETE FROM rac_pocetno_stanje WHERE godina='$godina' AND tip='ulazne'");

	 $rezultat= $mysqli->query("INSERT INTO rac_pocetno_stanje (godina,tip,iznos,korisnik) VALUES ('$godina','ulazne','$iznos','$korisnik')");
}
...
$godina=$_POST['godina'];
	 ...
	 FROM rac_fakture 
	 LEFT JOIN rac_roba  ON rac_fakture.usluga=rac_roba.id 		 
	 WHERE rac_fakture.aktivan=1 AND tip='ulaz' AND lice=$dobavljac AND godina=$godina)
AS leva_strana
...
(
	 ...
	 SELECT brdob,dat_izv,'00.00.0000','0.00',isplate,'',2 AS red,dobavljac
	 FROM rac_izvodi
	 WHERE rac_izvodi.aktivan=1  AND dobavljac=$dobavljac  AND godina=$godina
	...
	SELECT *,3,'' AS red  FROM (
		...
		FROM rac_fakture 
		LEFT JOIN rac_usluge ON rac_fakture.usluga=rac_usluge.id 		 
		WHERE rac_fakture.aktivan=1 AND tip='ulaz'  AND godina=$godina
		GROUP BY faktura_br
		UNION ALL
	 SELECT brdob,dat_izv,'00.00.0000','0.00',sum(isplate),'sum_platio' AS red
	 FROM rac_izvodi
...
ORDER BY datum_ulaza,faktura_br,red	,dat_fak
	 ");?>
		...
		<div class="box">
			...
			<?php
				...
				elseif ($row['n_roba']=="sum_duguje") {
				/* izmena */	
				 echo "<tr style='background:#A5AFFD'><td></td><td></td><td></td><td>".$row['n_roba']."</td><td>".$row['dat_fak']."</td><td><b>Duguje:</b></td><td><b>".number_format($row['iznos_fa'], 2, ',', '.')."</b></td></tr>";
				$medjuzbir_duguje=1;
				$v_medjuvrednost_duga=$row['iznos_fa'];
				}
				elseif ($row['n_roba']=="sum_platio")/* izmena */ {
					/* izmena */					
	...
	<input type="hidden" name=korisnik value=<?php echo $dobavljac ?> >
	<input type="hidden" name=dobavljaci value=<?php echo $_POST['dobavljaci'] ?> >	
	...
	<?php
		echo "<a href='izv_ulazne1.php?dobavljac=$dobavljac&godina=$akt_godina&naziv_dobavljaca=$naziv_dobavljaca' target='_blanc'><img src='../img/printer.png'></img></a>";
	}
	?>
	<script>
	...
	setTimeout(function(){	
		document.getElementById('fpoc_stanje').click()},1200)	
    }	
	</script>