...
if (isset($_POST['unesi'])){
	   ...
	   $rezultat= $mysqli->query("INSERT INTO rac_dev_fakture (faktura_br,dat_fak,dat_val,iznos_fa,porez,lice,usluga,napomena,tip,godina,aktivan) VALUES 
														('$faktura_br','$dat_fak','$dat_val','$iznos_fa','$porez','$kupci','$usluge','$napomena','izlaz',$akt_godina,1)");

		}
		if (isset($_POST['izmeni']))
		{
		$rezultat= $mysqli->query("UPDATE rac_dev_fakture SET godina='$akt_godina',dat_fak='$dat_fak',faktura_br='$faktura_br',dat_val='$dat_val',iznos_fa='$iznos_fa',porez='$porez',lice='$kupci',usluga='$usluge',napomena='$napomena' WHERE id='$id'");
	}
}	
	?>
	<!-- ############### pretraga ##############################-->
	...
	<form action="default.php" method=post>
		...
		<input type=hidden name=str value='dev_izlazne'>
	...
	if (isset($_REQUEST['del'])) {
		$rezultat= $mysqli->deaktiviraj($_REQUEST['del'],"rac_dev_fakture");
	...
	$_SESSION['dev_izlazne']=string2datum($_POST['dat_fakture']);
	}	
	if (isset($_SESSION['dev_izlazne'])){//zapamti datum fakture u $dat_izlaz_fakture
	$dat_izlaz_fakture=$_SESSION['dev_izlazne'];
	$dat_izlaz_fakture=date("Y-m-d", strtotime($dat_izlaz_fakture));
	if ($dat_izlaz_fakture=='1970-01-01') $dat_izlaz_fakture='%%';

		$pocetak=$mysqli->paginacija1("SELECT COUNT(ID) AS broj FROM rac_dev_fakture WHERE aktivan=1 AND tip='izlaz' AND dat_fak like '%$dat_izlaz_fakture%' AND godina='$akt_godina' ","dev_izlazne");
		$rezultat= $mysqli->query("SELECT *,rac_dev_fakture.id AS id1 FROM rac_dev_fakture LEFT JOIN rac_dev_kupci ON rac_dev_fakture.lice=rac_dev_kupci.id WHERE rac_dev_fakture.aktivan=1  AND rac_dev_fakture.tip='izlaz' AND dat_fak like '%$dat_izlaz_fakture%' AND godina='$akt_godina' LIMIT $pocetak,10");
	  	?>

		<div class="box">
			DEVIZNE ULAZNE FAKTURE<hr>
			<table class="lista" style="diplay:block;width:100%">
				...
				$link="default.php?str=dev_izlazne&id=$id";				
				echo "<tr class='lista'><td style='width:12%;text-align:center'><a href='$link'>".$row['faktura_br']."</a></td><td text-align:center'><a href='$link'>".$row['naziv']."</a><td style='text-align:center'><a href='$link'>".$dat_fak."</a></td><td ><a href='default.php?str=dev_izlazne&del=$id'>x</a></td></tr>";
				$i=$i+1;
				}
				?>
			</table>
			<br>
			<input type="button" value="Unos nove devizne izlazne fakture" onclick="sklopi('unesi')">
		</div>
<?php
		}
...
// a ako je korisnik kliknuo na odredjeni red iscitaj vrednosti iz baze
		...
		if (isset($_GET['id'])){
		$id=$_GET['id'];
		$rezultat= $mysqli->query("SELECT * FROM rac_dev_fakture WHERE id='$id' ");
		while ($row = $rezultat->fetch_assoc()) 
				{
				$id1=$row['id'];
...
<form method="post" action="default.php">
		...
		?><br>
		Iznos fakture:<br><input type="text" class='isplate' name="iznos_fa" value='<?php echo $iznos_fa1?>'><br>	
		...
		<input type="hidden" value=dev_izlazne name="str">
