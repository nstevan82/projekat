...
<input type=hidden name=str value='blagajna_gorivo_izlaz1'>
</form>
</div>
...
if (isset($_POST['unesi'])){
	   ...
	   $rezultat= $mysqli->query("INSERT INTO rac_blagajna_gorivo (godina,datum,opis,ulaz,opisu,objekat,tip,aktivan) VALUES ('$godina','$datum','$opis','$ulaz','$opisu','$objekat','izlaz',1)");
	...
	if (isset($_REQUEST['del'])) {
		...
		$rezultat= $mysqli->query("SELECT *,rac_objekti.naziv AS n_objekta FROM rac_blagajna_gorivo 
		...
		<div class="box">
			<table class="lista" style="diplay:block;width:100%">
				<tr><th>Rb</th><th>Datum</th><th>Opis</th><th>Iznos</th><th>Tip</th><th></th></tr>
				<?php
				$i=10*($_GET['strana'])+1;
				...
				echo "<tr class='lista'><td>".$i."</td><td><a href='$link'>".$datum."</a></td><td><a href='$link'>".$row['n_objekta']."</a></td><td><a href='$link'>".number_format($row['ulaz'], 2, ',', '.')."</a></td><td><a href='$link'>".$row['tip']."</a></td><td><a href='default.php?str=blagajna_gorivo&del=$id'>x</a></td></tr>";
				$i=$i+1;
				}
				?>
			</table>
...
<form method="post" action="default.php">
		...
		Izlaz:<br><input type="text" name="ulaz" value='<?php echo $ulaz1?>'><br>		
			
		<?php		
		$rezultat=$mysqli->query("SELECT * FROM rac_objekti WHERE aktivan=1");
		$mysqli->dropdown1($rezultat,'objekat',$objekat1,'id');
		?><br>
		<input type="hidden" value=blagajna_gorivo_izlaz1 name="str">
