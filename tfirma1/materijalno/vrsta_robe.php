...
if (isset($_POST['unesi'])){
	...
	$sifra=$_POST['sifra'];
	   ...
	   $rezultat= $mysqli->query("INSERT INTO rac_vrsta_robe (naziv,sifra,aktivan) VALUES ('$naziv','$sifra',1)");
		...
		$sifra=$_POST['sifra'];
		$id=$_POST['id'];
		$rezultat= $mysqli->query("UPDATE rac_vrsta_robe SET naziv='$naziv',sifra='$sifra' WHERE id='$id'");
	}
	if (isset($_REQUEST['del'])) {
		$rezultat= $mysqli->deaktiviraj($_REQUEST['del'],"rac_vrsta_robe");
		...
		$pocetak=$mysqli->paginacija1("SELECT COUNT(ID) AS broj FROM rac_vrsta_robe WHERE aktivan=1","vrsta_robe");

		$rezultat= $mysqli->query("SELECT * FROM rac_vrsta_robe WHERE aktivan=1 LIMIT $pocetak,10");
	  	?>
		...
		<div class="box">
			<table class="lista" style="diplay:block;width:100%">
				...
				$id=$row['id'];
				$naziv=$row['naziv'];
				$link="default.php?str=vrsta_robe&id=$id";
				
				echo "<tr class='lista'><td>".$i."</td><td><a href='$link'>".$row['naziv']."</a></td><td><a href='$link'>".$row['sifra']."</a></td><td><a href='default.php?str=vrsta_robe&del=$id'>x</a></td></tr>";
				$i=$i+1;
				}
				?>
			</table>
			<br>
			<input type="button" value="Unos nove vrste robe" onclick="sklopi('unesi')">
		</div>
...
// a ako je korisnik kliknuo na odredjeni red iscitaj vrednosti iz baze
		...
		if (isset($_GET['id'])){
		$id=$_GET['id'];
		$rezultat= $mysqli->query("SELECT * FROM rac_vrsta_robe WHERE id='$id'");
		while ($row = $rezultat->fetch_assoc()) 
				{
				$id1=$row['id'];
				$naziv1=$row['naziv'];
...
<form method="post" action="default.php">
		VRSTA ROBE<br>
		<hr>
		Naziv vrste robe:<br><input type="text" name="naziv" value='<?php echo $naziv1?>'><br>	
		Napomena:<br><input type="text" name="sifra" value='<?php echo $sifra1?>'><br>		
	
		<input type="hidden" value=vrsta_robe name="str">
