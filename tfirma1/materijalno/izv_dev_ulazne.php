...
<?php
		...
		<input type="hidden" name=str value="izv_dev_ulazne">
		...
		<?php		
/*
	 SELECT *,rac_usluge.naziv AS n_roba 
	 FROM rac_dev_fakture 
	 LEFT JOIN rac_usluge  ON rac_dev_fakture.usluga=rac_usluge.id 
	 LEFT JOIN rac_dev_izvodi ON rac_dev_fakture.faktura_br=rac_dev_izvodi.dobavljac   
	 WHERE rac_dev_fakture.aktivan=1 AND tip='izlaz' AND lice=$dobavljac
...
$datum_do=date("Y-m-d", strtotime($_POST['datum_do']));
	 ...
	 WHERE rac_dev_fakture.aktivan=1 AND tip='ulaz' AND lice=$dobavljac AND (dat_fak BETWEEN '2014-01-01' and '$datum_do'))
AS leva_strana
...
(
	 ...
	 FROM rac_dev_izvodi
	 WHERE rac_dev_izvodi.aktivan=1  AND dobavljac=$dobavljac  AND (dat_izv BETWEEN '2014-01-01' and '$datum_do')
...
ORDER BY datum_ulaza,faktura_br,red	,dat_fak 	
	 ");?>
		...
		<div class="box">
			...
			// izmena
			if ($row['dat_fak']<=$datum_do && $row['dat_fak']>=$datum_od) 	
	...
	<?php
			echo "<a href='izv_dev_ulazne1.php?dobavljac=$dobavljac&godina=$akt_godina&datum_do=$datum_do&datum_od=$datum_od' target='_blanc'><img src='../img/printer.png'></img></a>";
	}
	?>