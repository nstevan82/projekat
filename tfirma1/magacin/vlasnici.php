...
if (isset($_POST['unesi'])){
	$naziv=$_POST['naziv'];
	   ...
	   $rezultat= $mysqli->query("INSERT INTO mag_vlasnici (naziv,aktivan) VALUES ('$naziv',1)");
		...
		$id=$_POST['id'];
		$rezultat= $mysqli->query("UPDATE mag_vlasnici SET naziv='$naziv' WHERE id='$id'");
	}
	if (isset($_REQUEST['del'])) {
		$rezultat= $mysqli->deaktiviraj($_REQUEST['del'],"mag_vlasnici");
		...
		$pocetak=$mysqli->paginacija1("SELECT COUNT(ID) AS broj FROM mag_vlasnici WHERE aktivan=1","vlasnici");

		$rezultat= $mysqli->query("SELECT * FROM mag_vlasnici WHERE aktivan=1 LIMIT $pocetak,10");
	  	?>
		...
		<div class="box">
			<table class="lista" style="diplay:block;width:100%">
				...
				$id=$row['id'];
				$naziv=$row['naziv'];
				$link="default.php?str=vlasnici&id=$id";
				
				echo "<tr class='lista'><td>".$i."</td><td><a href='$link'>".$row['naziv']."</a></td><td><a href='default.php?str=vlasnici&del=$id'>x</a></td></tr>";
				$i=$i+1;
				}
				?>
			</table>
			<br>
			<input type="button" value="Unos novog vlasnici" onclick="sklopi('unesi')">
		</div>
...
// a ako je korisnik kliknuo na odredjeni red iscitaj vrednosti iz baze
		...
		if (isset($_GET['id'])){
		$id=$_GET['id'];
		$rezultat= $mysqli->query("SELECT * FROM mag_vlasnici WHERE id='$id'");
		while ($row = $rezultat->fetch_assoc()) 
				{
				$id1=$row['id'];
				$naziv1=$row['naziv'];				
...
<form method="post" action="default.php">
		vlasnici<br>		
		Naziv vlasnici:<br><input type="text" name="naziv" value='<?php echo $naziv1?>'><br>		
		<input type="hidden" value=vlasnici name="str">
