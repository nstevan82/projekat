...
<div class="box">
<?php
		$Vei0ij40kkwq=$Vkb4v4hyhajp->query("SELECT * FROM rac_dobavljaci WHERE aktivan=1");
		$Vkb4v4hyhajp->dropdown($Vei0ij40kkwq,'dobavljaci','dobavljaci','id');?>
...
$V24imcqdcpvk=$_POST['dobavljaci'];

$datum_od=date("Y-m-d", strtotime($_POST['datum_od']));
$datum_do=date("Y-m-d", strtotime($_POST['datum_do']));
	 $Vei0ij40kkwq= $Vkb4v4hyhajp->query("SELECT * FROM rac_dobavljaci WHERE id='$V24imcqdcpvk'");
	 ...
	 WHERE rac_fakture.aktivan=1 AND tip='ulaz' AND lice=$V24imcqdcpvk AND (dat_fak BETWEEN '$datum_od' and '$datum_do'))
AS leva_strana
...
(
	 ...
	 FROM rac_izvodi
	 WHERE rac_izvodi.aktivan=1  AND dobavljac=$V24imcqdcpvk AND (dat_izv BETWEEN '$datum_od' and '$datum_do')
...
ORDER BY datum_ulaza,faktura_br,red	,dat_fak
	...
	<?php
		echo "<a href='izv_ulazne1.php?dobavljac=$V24imcqdcpvk&godina=$V0qmlm2aq14j&naziv_dobavljaca=$Vb4jfmbhorvo&datum_do=$datum_do&datum_od=$datum_od' target='_blanc'><img src='../img/printer.png'></img></a>";
	}
	?>