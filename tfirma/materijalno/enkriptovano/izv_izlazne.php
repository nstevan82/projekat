...
<div class="box">
<?php
		$Vei0ij40kkwq=$Vkb4v4hyhajp->query("SELECT * FROM rac_kupci WHERE aktivan=1");
...
$datum_do=date("Y-m-d", strtotime($_POST['datum_do']));
	 $Vei0ij40kkwq= $Vkb4v4hyhajp->query("SELECT * FROM rac_kupci WHERE id='$V4rafkd3eibx'");
	 WHILE ($Vicgg1kpov5s=$Vei0ij40kkwq->fetch_assoc()){
	 $Vgxevhhjbl02=$Vicgg1kpov5s['naziv'];	 
	 }
...
WHERE rac_fakture.aktivan=1 AND tip='izlaz' AND rac_fakture.lice='$V4rafkd3eibx' AND (dat_fak BETWEEN '$datum_od' and '$datum_do')) AS ulazi 
right join 
( 
SELECT faktura_br,dat_fak,rac_fakture.dat_val,iznos_fa,'0.00' AS uplate,rac_usluge.naziv AS n_usluge,1 AS red,rac_fakture.lice,dat_fak as dat,1 AS red_izlaz 
FROM rac_fakture LEFT JOIN rac_usluge ON rac_fakture.usluga=rac_usluge.id 
WHERE rac_fakture.aktivan=1 AND tip='izlaz' AND rac_fakture.lice='$V4rafkd3eibx'  AND (dat_fak BETWEEN '$datum_od' and '$datum_do')
UNION ALL 
SELECT brfa,dat_izv,'00.00.0000','0.00',uplate,'',1 AS red,kupac,'',2 AS red_izlaz  
FROM rac_izvodi WHERE rac_izvodi.aktivan=1 AND kupac='$V4rafkd3eibx' AND (dat_izv BETWEEN '$datum_od' and '$datum_do')
UNION ALL 
...
ORDER BY dat_ulaza,izlazi.faktura_br,izlazi.red ,izlazi.red_izlaz,izlazi.dat_fak
	 ...
	 ?>
		<div class="box">
		<?php echo $Vgxevhhjbl02." na dan:".date("d.m.Y")."<br>"; 	?>
	...
	<?php
	echo "<a href='izv_izlazne1.php?kupac=$V4rafkd3eibx&godina=$V0qmlm2aq14j&naziv_kupca=$Vgxevhhjbl02&datum_do=$datum_do&datum_od=$datum_od' target='_blanc'><img src='../img/printer.png'></img></a>";
	}
	?>