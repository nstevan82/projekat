...
if (isset($_POST['unesi'])){
	   ...
	   $rezultat= $mysqli->query("INSERT INTO rac_dev_fakture (godina,faktura_br,dat_fak,dat_val,iznos_fa,porez,napomena,lice,usluga,tip,aktivan) VALUES ('$godina','$faktura_br','$dat_fak','$dat_val','$iznos_fa','$porez','$napomena','$dobavljaci','$roba','ulaz',1)");

		}
		if (isset($_POST['izmeni']))
		{
		$rezultat= $mysqli->query("UPDATE rac_dev_fakture SET godina='$godina',dat_fak='$dat_fak',faktura_br='$faktura_br',dat_val='$dat_val',iznos_fa='$iznos_fa',porez='$porez',napomena='$napomena',lice='$dobavljaci',usluga='$roba' WHERE id='$id'");
	}
}	
	...
	<form action="default.php" method=post>
		...
		<input type=hidden name=str value='dev_ulazne'>
	...
	$_SESSION['dev_ulazne']=string2datum($_POST['dat_fakture']);
	}	
	if (isset($_SESSION['dev_ulazne'])){//zapamti datum fakture u $dat_ulaz_fakture
	$dat_ulaz_fakture=$_SESSION['dev_ulazne'];
	$dat_ulaz_fakture=date("Y-m-d", strtotime($dat_ulaz_fakture));
	if ($dat_ulaz_fakture=='1970-01-01') $dat_ulaz_fakture='%%';
		
		
		$pocetak=$mysqli->paginacija1("SELECT COUNT(ID) AS broj FROM rac_dev_fakture WHERE aktivan=1 AND tip='ulaz' AND dat_fak like '%$dat_ulaz_fakture%' AND godina='$godina'","dev_ulazne");
		$rezultat= $mysqli->query("SELECT *,rac_dev_fakture.id AS id1 FROM rac_dev_fakture LEFT JOIN rac_dev_dobavljaci ON rac_dev_fakture.lice=rac_dev_dobavljaci.id  WHERE rac_dev_fakture.aktivan=1  AND tip='ulaz' AND dat_fak like '%$dat_ulaz_fakture%' AND godina='$godina' ORDER BY dat_fak ASC LIMIT $pocetak,10");
		?>

		<div class="box">
		DEVIZNE ULAZNE FAKTURE<br>
		<hr>
			<table class="lista" style="diplay:block;width:100%">
				...
				$link="default.php?str=dev_ulazne&id=$id";				
				echo "<tr class='lista'><td>".$i."</td><td><a href='$link'>".$row['naziv']."</a></td><td><a href='$link'>".$dat_fak."</a></td><td><a href='$link'>".$row['faktura_br']."</a></td><td><a href='default.php?str=dev_ulazne&del=$id'>x</a></td></tr>";
				$i=$i+1;
				}
				?>
			</table>
			<br>
			<input type="button" value="Unos nove devizne ulazne fakture" onclick="sklopi('unesi')">
		</div>
<?php 
}
...
<form method="post" action="default.php">
		...
		Dobavljac
		<?php		
		$rezultat=$mysqli->query("SELECT * FROM rac_dev_dobavljaci");
		...
		<input type="hidden" value=dev_ulazne name="str">
